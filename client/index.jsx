import axios from 'axios';
import React from 'react';
import ReactDOM from 'react-dom';
import $ from 'jquery';
import AverageScore from './AverageScore.jsx';


class App extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			totalReview: [],
			currentReview: [],
			currentPage: 1,
			amountReview: 0,
			totalPage: 0
		};

		this.handleButtonBack = this.handleButtonBack.bind(this);
		this.handleButtonForward = this.handleButtonForward.bind(this);
		this.getOnePage = this.getOnePage.bind(this);

		this.getOnePage();
	}

	handleButtonForward(e) {
		if (e !== undefined) {
			this.setState({
				currentPage: Number(e.target.id) + 1
			})
		}
	}

	handleButtonBack(e) {
		if (e !== undefined) {
			this.setState({
				currentPage: Number(e.target.id) - 1,
				currentReview: totalReview.slice(8, 15)
				// currentReview: totalReview.slice(Number(e.target.id) * 7, Number(e.target.id) * 7 + 7)
			})
		}
	}

	getOnePage() {
		axios.get(`/api/reviews/`)
			// .then(results => console.log('1', results.data[0]['name']))
			.then(results => 
			{
				this.setState
				({
					totalReview : results.data,
					// amountReview: results.data.length,
					totalPage: Math.ceil(results.data.length / 7),
					currentReview: results.data.slice(0, 7),
				})
			})
			// .then(()=>console.log('2', self.state.currentReview))
		  .catch(error => console.log('FeTcHiNg error: ', error))
	}
		
	render() {
		return (
			<div>
				{/* <p>{this.state.currentReview[0].review}</p> */}
				<div>
					<img src={"https://upload.wikimedia.org/wikipedia/commons/thumb/8/8f/Communist_star.svg/250px-Communist_star.svg.png"} id="stars"></img>
					<img src={"https://upload.wikimedia.org/wikipedia/commons/thumb/8/8f/Communist_star.svg/250px-Communist_star.svg.png"} id="stars"></img>
					<img src={"https://upload.wikimedia.org/wikipedia/commons/thumb/8/8f/Communist_star.svg/250px-Communist_star.svg.png"} id="stars"></img>
					<img src={"https://upload.wikimedia.org/wikipedia/commons/thumb/8/8f/Communist_star.svg/250px-Communist_star.svg.png"} id="stars"></img>
					<h2>Search Reviews</h2>
					<AverageScore total={this.state.totalReview} />

					{this.state.currentReview.map((x) => (
						<div key = {x['name'].toString() + Math.floor(Math.random()*(9999))}>
							<img src={`https://s3-us-west-1.amazonaws.com/errbnb/${x['id']}.jpg`} id = "avatar"></img>
							<h3 id = 'name'>{x['name']}</h3>
							<p id = 'date'>{x['date']}</p>
							<p id = 'review'>{x['review']}</p>
						</div>
					))}	

					{/* <p>Accuracy: {this.state.currentReview[0]['accuracy']}
					__________Location: {this.state.currentReview[0]['location']}</p>
					<p>Communication: {this.state.currentReview[0]['communication']}
					__________Checkin: {this.state.currentReview[0]['checkin']}</p>
					<p>Cleanliness: {this.state.currentReview[0]['cleanliness']}
					__________Value: {this.state.currentReview[0]['value']}</p>
					<img src={"https://s3-us-west-1.amazonaws.com/errbnb/3.jpg"} id = "avatar"></img>
					<h3 id = 'name'>{this.state.currentReview[0]['name']}</h3>
					<p id = 'date'>{this.state.currentReview[0]['date']}</p>
					<p id = 'review'>{this.state.currentReview[0]['review']}</p> */}
					<button type="button" aria-busy="false">
						<span>Read More</span>
					</button>
					<h4>Page {this.state.currentPage}</h4>		
					<h4>Total Page: {this.state.totalPage}</h4>
					<input type = "button" id = "1" value = "<" onClick={this.handleButtonBack}></input>
					<input type = "button" id = "1" value = ">" onClick={this.handleButtonForward}></input>
				</div>
			</div>
		)
		// console.log('second')
		// if (this.state.currentPage === 1) {
		// 	console.log('InDeX rendering ', this.state.totalReview)
		// 	return this.page1();
		// } else if (this.state.currentPage === 2) {
		// 	return this.page2();
		// } else if (this.state.currentPage === 3) {
		// 	return this.page3();
		// }
	}
}

ReactDOM.render(<App />, document.getElementById('app'));

